...
_DATA_ACQUISITION_DIRECTIVE = (
    ...
    "6. GET DATA BY STAGING IT, not by hand-rolling a live query. The RELIABLE "
    "path is [FETCHDATA:] — the orchestrator downloads with a size cap + schema "
    "card. For Gaia, `[FETCHDATA: gaia:gaiadr3.nss_two_body_orbit]` stages real "
    "orbits of ALL solution types with correct column names; stage APOGEE/GALAH "
    "the same way. Read the staged files from /data/shared/data. A server-side TAP "
    "JOIN in-experiment (`SELECT ... FROM gaiadr3.nss_two_body_orbit o JOIN "
    "gaiadr3.astrophysical_parameters p USING (source_id) WHERE ...`) is an "
    "OPTIONAL optimization to avoid a lossy local crossmatch — but a live query is "
...
_REPLICATOR_PROMPT = (
    ...
    "the conclusion rests on. State it in a comment. CLASSIFY it: is it an EFFECT "
    "claim (a non-zero slope/correlation/difference with a sign) or a NULL claim "
    "(no significant effect / an upper bound / 'vanishes under controls')? Print "
    "`RESULT[headline_is_null]=<1 if a null/no-effect/bound claim, else 0>`.\n"
    "2. RE-DERIVE the headline quantity YOURSELF from the raw staged data (the "
    "effect size, or for a null the same effect + its significance). Do NOT reuse "
    "the authors' numbers — recompute it. Print `RESULT[headline_recomputed]=<v>`.\n"
    "3. STRESS-TEST it under {max_specs} DIFFERENT but equally-defensible "
    "specifications a skeptical referee would try (a different sample cut, control "
    "variable, subgroup, or estimator). For EACH print `RESULT[spec_<shortname>_"
    "value]=<v>` and `RESULT[spec_<shortname>_sign]=<1 if positive, -1 if "
    "negative>`.\n"
    "4. Print `RESULT[n_specs]=<how many alternatives you tried>` and "
    "`RESULT[n_same_sign]=<how many kept the headline's sign>`.\n"
    "5. IF the headline is an EFFECT claim: print `RESULT[headline_reproduced]=<1 "
    "if your recomputed value has the SAME sign AND is within a factor of 2 of the "
    "paper's value, else 0>`.\n"
    "6. IF the headline is a NULL claim: the null is only credible if YOU also fail "
    "to find an effect. Print `RESULT[null_holds]=<1 if your independent re-"
    "derivation AND the MAJORITY of your specifications agree there is NO "
    "significant effect (the null is robust), or 0 if you find a significant effect "
    "the paper reported as null>`.\n\n"
    "Be adversarial and HONEST: if an effect flips sign or loses significance under "
    "reasonable choices — or if a claimed NULL actually hides a significant effect — "
    "THAT is the finding, report it. Read the real staged/cached data; never "
    "fabricate. Emit the script in a single ```python block."
)
...
_PHASE_INSTRUCTIONS: dict[ResearchPhase, dict[str, str]] = {
    ...
    ResearchPhase.INTERNAL_REVIEW: {
        "editor_review": (
            ...
            "Topic: {seed_prompt}\n\n"
            "## Paper Draft\n{current_draft}\n\n"
            "CRITICAL OUTPUT RULE: Emit the ## Recommendation, ## Blocking Changes, and "
            "## Minor Changes sections FIRST, before any praise — they are mandatory and "
            "must never be omitted. Keep Strengths/Weaknesses brief (a few bullets each). "
            "A review that lists strengths but no recommendation is INVALID. Use exactly "
            "## headers (not ###).\n\n"
            "**TIER EVERY ISSUE.** A change is BLOCKING only if it invalidates the "
            "science or the reader's trust: fabricated/untraceable numbers, claims "
            "contradicting the evidence, references to figures that do not exist, "
            "misattributed citations, internally contradictory results. Wording, "
            "rounding/precision presentation, label or sample-name clarity, stylistic "
            "structure, and consistency polish are MINOR — they will be applied in one "
            "final polish pass and must NEVER hold the paper back. If an issue does not "
            "change any conclusion, it is minor.\n\n"
            "**NO MOVING TARGETS.** Enumerate EVERY blocking issue in your FIRST review. "
            "On later iterations, only (a) verify previously required changes and "
            "(b) flag NEW problems introduced by the revision itself as blocking; a "
            "pre-existing nit you only just noticed goes under Minor Changes.\n\n"
            ...
            "ORDER — Recommendation and the change lists come FIRST so they survive even "
            "if your reply is cut short:\n"
            "## Recommendation\n- Exactly one word on its own line: 'accept' (no blocking "
            "issues — minor polish, if any, will be applied in a final pass), 'revise' "
            "(blocking issues exist that a revision can fix), or 'reject' (fundamentally "
            "flawed — would not pass peer review even with revisions). Do NOT recommend "
            "'revise' when every open issue is minor.\n\n"
            "## Blocking Changes\n- Science-invalidating issues only (write 'None' if "
            "there are none)\n\n"
            "## Minor Changes\n- Cosmetic/presentation polish: wording, rounding, labels, "
            "clarity (write 'None' if there are none)\n\n"
